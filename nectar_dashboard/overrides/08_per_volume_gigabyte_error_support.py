...
from openstack_dashboard.dashboards.project.volumes.volumes import forms as volume_forms
...
# Report the error when a user is unable to create a volume larger than what
# their per_volume_gigabytes quota is set to.
# This overwrites the original class. The entire original class must be copied
# and pasted here, then modifications can be done.
class CyberaVolumeCreateForm(forms.SelfHandlingForm):
    ...
    def __init__(self, request, *args, **kwargs):
        super(CyberaVolumeCreateForm, self).__init__(request, *args, **kwargs)
        volume_types = []
        try:
    ...
    def clean(self):
        cleaned_data = super(CyberaVolumeCreateForm, self).clean()
    ...
    def handle(self, request, data):
        try:
            ...
            return False
        # jt
        # Return the per_volume_gigabytes error
        except Exception as e:
            m = "%s" % e
            message = _("Unable to create volume.")
            if "VolumeSizeExceedsLimit" in m:
                message = m
...
volume_forms.CreateForm = CyberaVolumeCreateForm
