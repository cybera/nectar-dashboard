...
class IdentityAPIVersionManager(base.APIVersionManager):
    ...
    def get_project_manager_projects(self, *args, **kwargs):
    ...
    def get_project_manager_users(self, *args, **kwargs):
        return keystoneclient(*args, **kwargs).users
...
def tenant_create(request, name, description=None, enabled=None,
                  domain=None, **kwargs):
    manager = VERSIONS.get_project_manager_projects(request, admin=True)
    if VERSIONS.active < 3:
...
def tenant_get(request, project, admin=True):
    manager = VERSIONS.get_project_manager_projects(request, admin=admin)
